...
We want to re-write this for both speed and maximum flexibilty
As annoying as it might be, the fastest way to load ERDDAP data is to only load relevant sets
And let the date reader deal with itself

TODO:
    Generate urls which will load specific variables
    Should give capacity to load in specific dates but should not be the default reader
    Possible variables will need to be determined from .das file rather than .csv directly

Ideas:
    Give the user the ability to see data which is beyond the stated scope
...
'''
Interesting jag, but I dont' remember why I was doing this. More automation, probably
